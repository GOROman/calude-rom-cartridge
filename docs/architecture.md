...
- CHR SRAM(M68AF127B, 128KB)はA15/A16をLow固定し、現設計では最大4バンク(32KB CHR)まで使用。対象ゲーム `calude-famicom-game` はCNROM/PRG32KB/CHR16KB(2バンク)/垂直ミラー
...
```
   ...
   CPU側                                    PPU側
    ...
    D0-D7 ←[74HCT245]←+              |       PD0-PD7 ←[74HCT245]←+  |
                      |              |                           |  |
               +------+--------------+--+              +---------+--+----+
               | U1 PRG SRAM M68AF127B   |              | U2 CHR SRAM     |
               +-----------+------------+              +--------+--------+
                           |                                    |
...
| Ref | 部品 | LCSC候補 |
|---|---|---|
| U1,U2 | M68AF127B (128K×8 SRAM, 4.5-5.5V, 55ns, SOP32) | 秋月電子 101634（5個入）/ 101083（1個） |
...
- 段付き外形の最大寸法 **90 × 56.8 mm**、主要部 **90 × 46.1 mm**、板厚 **1.2mm** (1.6mmはコネクタを痛めるため不可)
- J3の60ピン・デバッグ用メスコネクタはバスを1:1でパススルーする。基板外形はケース互換のままだが、実装高さがあるためケース組み込み時はDNP
